...
class ArithmeticCoder:
    ...
    def __init__(self,
                 ...
                 hosting = None, # hosting method. "huggingface", "openai", "vllm",
        ...
        model_name = model_huggingface_id
    ...
    def _next_token_probs(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        if self.cache is None or val is None:            
            ...
            with torch.no_grad():
                ...
                output = self.lm.completions.create(
                ...
                probs_list = logprobs.token_logprobs
                tokens_list = logprobs.tokens
                text = output.choices[0].text
                #print(f"output.choices[0]={output.choices[0]}")
                #print(f"output.choices[0].logprobs={output.choices[0].logprobs}")
                #print(f"output.choices[0].logprobs.token_logprobs={output.choices[0].logprobs.token_logprobs[0:2]}")
                #print(f"output.choices[0].tokens={output.choices[0].logprobs.tokens[0:2]}")
                #print(f"output.choices[0].top_logprobs={output.choices[0].logprobs.top_logprobs[0:2]}")
                logger.debug(f"probs.len={len(probs_list)}")
                logger.debug(f"text={text}")
                logger.debug(f"top_logprobs={top_logprobs}")
                print(f"probs_list = {probs_list}({len(probs_list)})")
