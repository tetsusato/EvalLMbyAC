...
@app.cell
def _(an, leaderboard_df, pl, runs_df):
    # paper-07はモデル数を減らした実験なので，paper-05だとフルのモデルになるはず
    runs_df_sample = an.search_model_from_mlflow(["%"], run_name="fit2025-paper-05%")
    print(f"sample={runs_df_sample}")
    print(f"original={runs_df}")
    # "tags.model_name" をキーに結合
    runs_df_sample = runs_df_sample.join(
...
@app.cell
def _(df_report, linear_model, pl, poly_extractor):
...
@app.cell
def _(X, X_poly, model_poly):
    print(f"X={X}")
    print(f"X_poly={X_poly}") # = poly.fit_transform(X)
...
@app.cell
def _(X, model):
    model.predict(X[0:3, :].reshape(-1,2))
    return
